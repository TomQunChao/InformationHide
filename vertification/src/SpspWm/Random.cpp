...
double BoxMuller::extract() {
	double theta = m2->extractDouble3();
	theta = theta * PI * 2;
	double r = m1->extractDouble3();
	r = sqrt(-2 * log(r));
	return r * cos(theta);
}
BoxMuller::~BoxMuller() {
...
GgdGen::GgdGen(double sigmax,long s1,long s2,long s3,long s4,long s5) {
	m1 = new Mt19937(s1);
	m2 = new Mt19937(s2);
	double beta = getBeta(sigmax, 0.5);
	e1 = new ExpGen(s3,pow(1/beta,0.5));
	e2 = new ExpGen(s4, pow(1/beta, 0.5));
	beta = getBeta(sigmax, 1);
	e3 = new ExpGen(s5, 1/beta);
}
double GgdGen::getBeta(double sigmax,double c) {
	return 1 / sigmax * sqrt(tgamma(3 / c) / tgamma(1 / c));
}
...
double Random::extract() {
	return 0;
}
double Random::extract(double low, double high) {
	return low;
}
Random::Random() {
